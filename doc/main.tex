...
\newline\newline
Il semble maintenant important de savoir ce qu'est l'expected goal\footnote{Très souvent réduit par xG}. 
...
Il y en a également d'autres comme les expected points qui sont les points qu'une équipe devrait avoir gagnés, ceci basé sur les données relatives aux xG. D'autres dérivées sont indiquées sur l'article de Pinnacle écrit par Luke Petty. \cite{pettyWhatExpectedGoals2018a}
\newline\newline
...
Ils observent la métrique lors des derniers matchs et la compare avec le score réel du match. Si la différence est grande, cela peut permettre de constater un manque de réalisme ou un sur-régime d'une des deux équipes. \cite{tennerelBienUtiliserExpected2022a}
\newline \newline
...
Par exemple, l'identifiant de la base de données d'un tir pourrait être supprimé car il n'appporte rien pour la prédiction de l'expected goal.
...
Concernant les autres facteurs, ils servent à améliorer la prédiction mais rien n'indique s'ils ont autant d'influence que la distance et l'angle.
...
\begin{table}[htp]
    ...
    Perceptron multi-couches & 0.2888          \\ \hline
    Arbre de décision        & 0.2900         \\ \hline
    KNN                      & 0.4304         \\ \hline
...
De ce fait, nous pouvons déduire que ce modèle n'est pas le plus approprié pour ce problème.
...
Il en va de même pour l'arbre de décision qui était très similaire au perceptron multi-couches en termes de log loss mais qui possède une distribution de résultats prédits très différente.
\newline\newline
Finalement, nous observons que le classificateur naïf prédit systématiquement la classe majoritaire (qui est un tir manqué), ce qui était prévisible puisque cela représente le principe de ce modèle.
\begin{figure}[htp]
    \centering
...
Nous voyons notamment que malgré le fait que la log loss du perceptron multi-couches soit plus élevée que le modèle Random Forest, sa visualisation en heatmap est plus proche de la réalité que le modèle Random Forest qui lui fournit quelques résultats incohérents.
\newline\newline
